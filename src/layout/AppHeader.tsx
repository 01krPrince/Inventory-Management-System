import React from "react";
// Assuming TabProvider and useTabs are in '../context/TabContext'
import { TabProvider, useTabs } from '../context/TabContext'; 
// Assuming TabBar and AppSidebar are correctly exported from their files
import { TabBar } from './TabBar'; 
import { AppSidebar } from './AppSidebar'; 

// --- MOCK CONTEXTS/UTILITIES (Used for simplicity, replace with real context/router hooks) ---
const useSidebar = () => ({
    isExpanded: false,
    isHovered: false,
    isMobileOpen: false 
});
const Backdrop = () => null; 
// Mock Link component for the header
const Link = ({ children }: { children: React.ReactNode }) => <a href="#">{children}</a>;
const ThemeToggleButton = () => null;
const NotificationDropdown = () => null;
const UserDropdown = () => null;
// --- END MOCK CONTEXTS/UTILITIES ---

// --- COMPONENT MAP (The content for each tab) ---
const Welcome = () => <div className="p-6 mt-6 bg-white dark:bg-gray-700 rounded-lg shadow-md">👋 Welcome to Dashboard!</div>;
const Customer = () => <div className="p-6 mt-6 bg-white dark:bg-gray-700 rounded-lg shadow-md">👤 Customer Management Page</div>;

const ComponentMap: { [key: string]: React.FC } = {
    '/dashboard': Welcome,
    '/customer': Customer,
    '/price-list': () => <div className="p-6 mt-6 bg-white dark:bg-gray-700 rounded-lg shadow-md">💰 Price List Configuration</div>,
    '/party-sale-discount-rate': () => <div className="p-6 mt-6 bg-white dark:bg-gray-700 rounded-lg shadow-md">👥 Party Discount View</div>,
    '/brandwise-discount-charges': () => <div className="p-6 mt-6 bg-white dark:bg-gray-700 rounded-lg shadow-md">🏷️ Brandwise Discount/Charges View</div>,
    '/estimate': () => <div className="p-6 mt-6 bg-white dark:bg-gray-700 rounded-lg shadow-md">🧾 Estimate Generator</div>,
    '/calendar': () => <div className="p-6 mt-6 bg-white dark:bg-gray-700 rounded-lg shadow-md">📅 Calendar View Component</div>,
    '/profile': () => <div className="p-6 mt-6 bg-white dark:bg-gray-700 rounded-lg shadow-md">⚙️ User Profile Settings</div>,
    '/form-elements': () => <div className="p-6 mt-6 bg-white dark:bg-gray-700 rounded-lg shadow-md">📄 Form Elements View</div>,
    '/basic-tables': () => <div className="p-6 mt-6 bg-white dark:bg-gray-700 rounded-lg shadow-md">📊 Basic Tables View</div>,
    '/blank': () => <div className="p-6 mt-6 bg-white dark:bg-gray-700 rounded-lg shadow-md">🔲 Blank Page View</div>,
    '/error-404': () => <div className="p-6 mt-6 bg-red-100 text-red-700 dark:bg-red-900 dark:text-red-300 rounded-lg">⚠️ 404 Error Page</div>,
    '/fallback': () => <div className="p-6 mt-6 bg-red-100 text-red-700 dark:bg-red-900 dark:text-red-300 rounded-lg">❗ Component not found for this tab path.</div>
};

// ============================================
// APP HEADER COMPONENT
// ============================================
export const AppHeader: React.FC = () => {
    const [isApplicationMenuOpen, setApplicationMenuOpen] = useState(false);

    const toggleApplicationMenu = () => {
        setApplicationMenuOpen(!isApplicationMenuOpen);
    };

    const inputRef = useRef<HTMLInputElement>(null);

    useEffect(() => {
        const handleKeyDown = (event: KeyboardEvent) => {
            if ((event.metaKey || event.ctrlKey) && event.key === "k") {
                event.preventDefault();
                inputRef.current?.focus();
            }
        };

        document.addEventListener("keydown", handleKeyDown);

        return () => {
            document.removeEventListener("keydown", handleKeyDown);
        };
    }, []);

    return (
        // The header needs to occupy the full width of the main content area
        <header className="sticky top-0 flex w-full bg-white border-gray-200 z-99999 dark:border-gray-800 dark:bg-gray-900 lg:border-b">
            <div className="flex flex-col items-center justify-between grow lg:flex-row lg:px-6">
                <div className="flex items-center justify-between w-full gap-2 px-3 py-3 border-b border-gray-200 dark:border-gray-800 sm:gap-4 lg:justify-normal lg:border-b-0 lg:px-0 lg:py-4">

                    <div className="flex-shrink-0 ml-3 justify-center p-4 h-16">
                        <span className="text-2xl font-bold text-gray-900 dark:text-white">Inventory</span>
                    </div>

                    <Link to="/" className="lg:hidden">
                        {/* Mocked Logo */}
                        <div className="w-8 h-8 bg-indigo-500 rounded-full dark:bg-indigo-300" />
                    </Link>

                    <button
                        onClick={toggleApplicationMenu}
                        className="flex items-center justify-center w-10 h-10 text-gray-700 rounded-lg z-99999 hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-gray-800 lg:hidden"
                    >
                        {/* Mock Menu Icon */}
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                            <path fillRule="evenodd" clipRule="evenodd" d="M5.99902 10.4951C6.82745 10.4951 7.49902 11.1667 7.49902 11.9951V12.0051C7.49902 12.8335 6.82745 13.5051 5.99902 13.5051C5.1706 13.5051 4.49902 12.8335 4.49902 12.0051V11.9951C4.49902 11.1667 5.1706 10.4951 5.99902 10.4951ZM17.999 10.4951C18.8275 10.4951 19.499 11.1667 19.499 11.9951V12.0051C19.499 12.8335 18.8275 13.5051 17.999 13.5051C17.1706 13.5051 16.499 12.8335 16.499 12.0051V11.9951C16.499 11.1667 17.1706 10.4951 17.999 10.4951ZM13.499 11.9951C13.499 11.1667 12.8275 10.4951 11.999 10.4951C11.1706 10.4951 10.499 11.1667 10.499 11.9951V12.0051C10.499 12.8335 11.1706 13.5051 11.999 13.5051C12.8275 13.5051 13.499 12.8335 13.499 12.0051V11.9951Z" />
                        </svg>
                    </button>

                    <div className="hidden lg:block">
                        <form>
                            <div className="relative">
                                {/* Search Icon */}
                                <span className="absolute -translate-y-1/2 pointer-events-none left-4 top-1/2">
                                    <svg className="fill-gray-500 dark:fill-gray-400" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path fillRule="evenodd" clipRule="evenodd" d="M3.04175 9.37363C3.04175 5.87693 5.87711 3.04199 9.37508 3.04199C12.8731 3.04199 15.7084 5.87693 15.7084 9.37363C15.7084 12.8703 12.8731 15.7053 9.37508 15.7053C5.87711 15.7053 3.04175 12.8703 3.04175 9.37363ZM9.37508 1.54199C5.04902 1.54199 1.54175 5.04817 1.54175 9.37363C1.54175 13.6991 5.04902 17.2053 9.37508 17.2053C11.2674 17.2053 13.003 16.5344 14.357 15.4176L17.177 18.238C17.4699 18.5309 17.9448 18.5309 18.2377 18.238C18.5306 17.9451 18.5306 17.4703 18.2377 17.1774L15.418 14.3573C16.5365 13.0033 17.2084 11.2669 17.2084 9.37363C17.2084 5.04817 13.7011 1.54199 9.37508 1.54199Z" fill="currentColor" />
                                    </svg>
                                </span>
                                <input
                                    ref={inputRef}
                                    type="text"
                                    placeholder="Search or type command..."
                                    className="dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-200 bg-transparent py-2.5 pl-12 pr-14 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-800 dark:bg-gray-900 dark:bg-white/[0.03] dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800 xl:w-[430px]"
                                />

                                <button className="absolute right-2.5 top-1/2 inline-flex -translate-y-1/2 items-center gap-0.5 rounded-lg border border-gray-200 bg-gray-50 px-[7px] py-[4.5px] text-xs -tracking-[0.2px] text-gray-500 dark:border-gray-800 dark:bg-white/[0.03] dark:text-gray-400">
                                    <span> ⌘ </span>
                                    <span> K </span>
                                </button>
                            </div>
                        </form>
                    </div>
                </div>
                <div
                    className={`${
                        isApplicationMenuOpen ? "flex" : "hidden"
                    } items-center justify-between w-full gap-4 px-5 py-4 lg:flex shadow-theme-md lg:justify-end lg:px-0 lg:shadow-none`}
                >
                    <div className="flex items-center gap-2 2xsm:gap-3">
                        <ThemeToggleButton />
                        <NotificationDropdown />
                    </div>
                    <UserDropdown />
                </div>
            </div>
        </header>
    );
};


// ============================================
// LAYOUT CONTENT
// ============================================
const LayoutContent: React.FC = () => {
    // Get sidebar context (mocked) and tab context
    const { isMobileOpen } = useSidebar();
    const { openTabs, activeTabPath, addTab } = useTabs(); 
    
    // We assume the sidebar is always fixed at 110px width
    const fixedSidebarWidthClass = "lg:ml-[110px]"; 
    
    return (
        <>
            {/* Scrollbar-hiding CSS */}
            <style>
                {`
                .hidden-scrollbar::-webkit-scrollbar {
                    display: none;
                }
                .hidden-scrollbar {
                    -ms-overflow-style: none; /* IE and Edge */
                    scrollbar-width: none; /* Firefox */
                }
                `}
            </style>
            
            <div className="min-h-screen bg-gray-100 dark:bg-gray-900 xl:flex">
                {/* Fixed Sidebar - Width: 110px */}
                <div>
                    <AppSidebar addTab={addTab} activeTabPath={activeTabPath} /> 
                    <Backdrop />
                </div>
                
                {/* Main Content Area - Must respect the sidebar's width */}
                <div
                    className={`flex-1 transition-all duration-300 ease-in-out ${
                        fixedSidebarWidthClass 
                    } ${isMobileOpen ? "ml-0" : ""}`}
                >
                    {/* Header is part of the main content flow */}
                    <AppHeader /> 
                    
                    {/* Tab Bar component */}
                    <TabBar />
                    
                    {/* Component Rendering Area (Persistent Tabs) */}
                    <div className="p-4 mx-auto max-w-[1536px] md:p-6">
                        {openTabs.map(tab => {
                            const TabComponent = ComponentMap[tab.path] || ComponentMap['/fallback'];
                            return (
                                // Render all open tabs, but only display the active one
                                <div 
                                    key={tab.path} 
                                    // Use absolute positioning or similar for perfect persistence if needed, 
                                    // but hidden/block is simple and effective for hiding components.
                                    className={tab.path === activeTabPath ? 'block' : 'hidden'}
                                >
                                    <TabComponent />
                                </div>
                            );
                        })}
                    </div>
                </div>
            </div>
        </>
    );
};

// ============================================
// MAIN WRAPPER
// ============================================
const AppLayout: React.FC = () => {
    return (
        // The TabProvider wraps the entire layout to provide state
        <TabProvider>
            <LayoutContent />
        </TabProvider>
    );
};

export default AppLayout;