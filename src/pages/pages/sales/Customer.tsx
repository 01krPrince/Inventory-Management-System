...
// --- AddCustomerModal (Remains Unchanged) ---
...
// --- ResizableHeader (Updated for Drag-and-Drop) ---
const ResizableHeader = ({
  col,
  requestSort,
  renderSortIndicator,
  setColumnWidths,
  columnWidths, // NEW PROPS for Reordering
  onDragStart,
  onDragOver,
  onDrop,
}) => {
  const thRef = useRef(null);
  const [isDragging, setIsDragging] = useState(false); // Initial width setting using inline style
  ...
  ); // Handler for the TH element click (Sorting)
  ...
  const handleDragStart = (e) => {
    ...
    if (e.target.className.includes("cursor-col-resize")) return;
  ...
  const handleDrop = (e) => {
    ...
    // Note: The logic for determining the insertion point (before/after)
    // is often complex. Here we rely on the parent component's onDrop logic.
  };
  ...
  return (
    <th
      ...
      onDrop={handleDrop}
      onDragEnd={handleDragEnd} // Tailwind classes for style and behavior
...
export default function Customer() {
  ...
  // NEW STATE: Tracks the current column order
  const [currentColumns, setCurrentColumns] = useState(employeeColumns);
  const dragItem = useRef(null); // Ref for the column being dragged
  const dragOverItem = useRef(null); // Ref for the column being dropped over
  ...
  const {
    ...
    currentColumns,
    initialPageSize
  ); /* --- Column Reordering Logic --- */ // Use currentColumns here
  ...
  const handleDrop = useCallback(
    ...
    [currentColumns]
  ); /* --- Data/Row Handlers (Mostly Unchanged) --- */
  ...
  const handleUpdateCustomer = (updatedData) => {
    ...
    setData((prevData) =>
      prevData.map((row) => (row.id === updatedData.id ? cleanedData : row))
    );
  };
      ...
      <div className="bg-white p-6 rounded-xl shadow-lg dark:bg-gray-800 border border-gray-200 dark:border-gray-700 w-full">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-4 space-y-4 md:space-y-0">
          ...
          <div className="flex items-center space-x-3 w-full md:w-auto justify-end">
            ...
            <button
              onClick={
                () => handleExport(data, currentColumns, "EmployeeDirectory") // Use currentColumns for export
              }
              className="p-2 text-gray-600 dark:text-gray-300 border border-gray-300 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 transition"
