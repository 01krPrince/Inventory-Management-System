...
>(({ currentItems, onExpenseChange }, ref) => {
  ...
  const [payments, setPayments] = useState<
    { ledger: string; amount: number; remarks: string }[]
  >([]);
  ...
  const calculateFinalDocAmount = () => {
    ...
    const finalTotal =
      taxable + tax + transport + adjustment + roundOff - otherDisc;
  ...
  useEffect(() => {
    const calculateFooterTotals = () => {
      ...
      if (itemValueRef.current)
        itemValueRef.current.value = totalItemValue.toFixed(2);
      if (taxableRef.current)
        taxableRef.current.value = totalTaxable.toFixed(2);
      if (taxAmountRef.current)
        taxAmountRef.current.value = totalTaxAmount.toFixed(2);
