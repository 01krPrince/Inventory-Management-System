...
import PostingGLPopup from "../../../../components/PostingGLPopup";

// --- INTERNAL COMPONENT: DATE INPUT (MODIFIED) ---
type DateInputProps = {
  value: string;
...
const DateInput: React.FC<DateInputProps> = ({ value, onChange }) => {
  ...
  const handleTriggerClick = (e: React.MouseEvent) => {
    e.stopPropagation();
    if (inputRef.current) {
      // Use showPicker if available (modern browsers), otherwise focus
  ...
  return (
    <div className="w-full h-full flex items-center justify-center relative group px-1">
      {/* Hidden Input: Keeps functionality but removes the default UI */}
      <input
        ref={inputRef}
        type="date"
        value={value || ""}
      ...
      {value ? (
        // IF DATE SELECTED: Show the text
        <span
          onClick={handleTriggerClick}
          className="text-xs cursor-pointer hover:text-blue-600 w-full text-center whitespace-nowrap overflow-hidden text-ellipsis"
          title={value} // Tooltip shows full date
        >
          {value}
        </span>
      ) : (
        // IF NO DATE: Show the Icon
        <button
          type="button"
          onClick={handleTriggerClick}
          className="flex items-center justify-center p-1.5 rounded-sm text-gray-500 hover:text-white hover:bg-[var(--theme-primary)] transition-all"
          title="Select Date"
        >
      ...
      {/* Optional: Styles to ensure hover effect uses the theme color correctly for the button */}
      <style>{`
        .group:hover button {
...
const OrderTable: React.FC<OrderTableProps> = ({
  ...
  const [glAccounts, setGlAccounts] = useState<SalesAndPurchaseGL[]>([]);
  ...
  // Posting GL Popup State
  const [glPopupState, setGlPopupState] = useState<{
    visible: boolean;
    top: number;
    left: number;
    activeRowId: string | null;
  ...
  const loadMasterData = async () => {
    try {
      ...
      if (Array.isArray(glData)) setGlAccounts(glData);
    } catch (error) {
  ...
  const closeGlPopup = () => {
    setGlPopupState((prev) => ({ ...prev, visible: false, activeRowId: null }));
  };
  ...
  const handleGLSelect = (gl: SalesAndPurchaseGL) => {
    if (glPopupState.activeRowId) {
      setTableData((prev) => ({
        ...prev,
        [glPopupState.activeRowId!]: {
          ...prev[glPopupState.activeRowId!],
          postinggl: gl.name,
        },
      }));
      setGlPopupState((prev) => ({
        ...prev,
        visible: false,
        activeRowId: null,
      }));
  ...
  return (
    ...
    >
                ...
                <tbody>
                  {sortedRowIds.map((rowId, vIdx) => {
                    ...
                    return (
                      <tr
                        key={rowId}
                        className="h-4 border-b hover:bg-blue-50"
                        style={{ borderColor: COLORS.border }}
                      >
                        {columns.map((col, cIdx) => {
                          ...
                          } else if (col.id === "postinggl") {
                            content = (
                              <div
                                className="text-[10px] italic text-gray-400 flex justify-between cursor-pointer hover:bg-gray-100 h-full items-center px-1"
                                onClick={(e) => handlePostingGLClick(e, rowId)}
                              >
                                {rowData.postinggl || "Select GL..."}{" "}
                                <span>▶</span>
                              </div>
                            );
                          } else if (col.id === "servicedate") {
                            content = (
                              <div className="relative w-full h-full">
                                <DateInput
                                  value={(rowData.servicedate as string) || ""}
                                  onChange={(e) =>
                                    handleInputChange(
                                      rowId,
                                      "servicedate",
      ...
      {/* --- POSTING GL POPUP --- */}
      <PostingGLPopup
        visible={glPopupState.visible}
        top={glPopupState.top}
        left={glPopupState.left}
        data={glAccounts}
        onClose={closeGlPopup}
