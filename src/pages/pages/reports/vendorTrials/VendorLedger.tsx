  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (response.success && Array.isArray(response.data)) {
        // Filter out the summary rows generated by the backend so we can control UI totals manually
        const actualTransactions = response.data.filter((r: any) => {
          const desc = r.item_description || "";
          return (
            !desc.includes("Total : For the period") &&
            !desc.includes("Grand Total")
          );
        });

        const mappedRows: LedgerRow[] = actualTransactions.map((txn: any) => {
          // Account for inconsistent API property casing (Date vs date)
            ...
            {paginatedData.length > 0
              ? paginatedData.map((row, i) => (
                  ...
                  >
                    ...
                    <td
                      className="border-r px-2 text-gray-600 max-w-[200px] truncate"
                      title={row.description}
                    >
                      {row.description}
                    </td>
                    <td className="border-r px-2 text-gray-500">
                      {row.stateName}
                    </td>
                    ...
                    <td
                      className="border-r px-2 text-gray-500 max-w-[200px] truncate"
                      title={row.address}
                    >
                      {row.address}
                    </td>
          ...
          {rows.length > 0 && (
            <tfoot className="sticky bottom-0 z-20 border-t-2 border-white/20 bg-[#0c5888] text-[11px] font-bold text-white shadow-lg">
              <tr>
                ...
                {/* 7 Columns: Date, VNo, Desc, State, PartyCode, GST, Address */}
                <td
                  colSpan={7}
                  className="px-4 py-2 text-right uppercase tracking-wider"
                >
                  Grand Total:
                </td>
