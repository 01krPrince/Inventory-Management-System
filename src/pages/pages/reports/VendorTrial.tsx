...
import {
  ...
  Users, // Changed icon to Users for Vendors
  Loader2,
  Filter, // Added for visual fidelity to screenshot
} from 'lucide-react';

// Placeholder for your actual service import
// import vendorTrialService, { IVendorTrialParams } from '../../../services/reports/vendorTrial';

// --- MOCK SERVICE (Remove this when integrating real API) ---
const mockService = {
  getAllVendorTrials: async (params: any) => {
    console.log(params);
    return new Promise<{ data: any[] }>((resolve) => {
      setTimeout(() => {
        resolve({
          data: Array.from({ length: 45 }).map((_, i) => ({
            group: i % 3 === 0 ? 'Local Suppliers' : 'Inter-State Suppliers',
            name: `Supplier Entity ${i + 1}`,
            code: `SUP-${1000 + i}`,
            contactPerson: `Manager ${i + 1}`,
            cellNo: `98765432${i < 10 ? '0' + i : i}`,
...
// Interface for filters (adjust based on your actual API)
interface IVendorTrialParams {
  storeCode?: string;
  fromDate?: string;
  toDate?: string;
}
...
export default function VendorTrial() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Replace mockService with your actual service call
      const response = await mockService.getAllVendorTrials(cleanParams);

      const mappedData: VendorTrialItem[] = (response.data || []).map((raw) => ({
        group: raw.group || 'General Suppliers',
