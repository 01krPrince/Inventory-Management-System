...
const OrderTable: React.FC<OrderTableProps> = ({
  ...
  const initialColumns: Column[] = [
    ...
    { id: "attr", label: "Attr", width: 40, align: "center", resizable: true },
    { id: "widg", label: "Widg", width: 40, align: "center", resizable: true },
    {
      id: "batch",
      label: "Batch",
      width: 45,
    ...
    { id: "igst", label: "IGST", width: 70, align: "right", resizable: true },
    { id: "cgst", label: "CGST", width: 70, align: "right", resizable: true },
    { id: "sgst", label: "SGST", width: 70, align: "right", resizable: true },
    {
      id: "minrate",
      label: "Min Rate",
      width: 70,
  ...
  const handleInputChange = (rowId: string, columnId: string, value: any) => {
    setTableData((prev) => ({
      ...prev,
  ...
  const handleDeleteRow = (rowIdToDelete: string) => {
    if (rows.length > 15) {
      ...
      setTableData((prev) => ({ ...prev, [rowIdToDelete]: {} }));
    }
  };
  ...
  const totals = useMemo(() => {
    ...
    return Object.fromEntries(
      Object.entries(sums).map(([k, v]) => [k, v.toFixed(2)])
    );
  }, [rows, tableData]);
      ...
      <div className="w-full">
        <div className="bg-white p-6 rounded-xl shadow-lg dark:bg-gray-800 border border-gray-200 dark:border-gray-700">
          <div className="mb-4">
            ...
            >
              <ArrowLeft className="w-4 h-4 mr-1" /> Back to Item Master
            </button>
          </div>
          <AddNewItem
                    ...
                    return (
                      ...
                      >
                        {columns.map((col, cIdx) => {
                          ...
                          else if (col.id === "return")
                            content = (
                              <input
                                type="checkbox"
                                checked={!!rowData.return}
                                onChange={(e) =>
                                  handleInputChange(
                                    rowId,
                                    "return",
                ...
                <tfoot className="sticky bottom-0 z-20 bg-gray-50">
                  <tr className="h-9 font-bold">
                    {columns.map((col, idx) => {
                      ...
                      else if (totals[col.id]) content = totals[col.id];
