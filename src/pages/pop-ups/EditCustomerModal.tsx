 ...
 */
export default function EditCustomerModal({ isOpen, onClose, rowData, onUpdate }) {
    ...
    const handleSubmit = (e) => {
        ...
        const requiredKeys = Object.keys(formData).filter(key => key !== 'id' && !formData[key]);
    ...
    const gridCols = dataKeys.length > 4 ? "grid-cols-1 md:grid-cols-2" : "grid-cols-1";
    ...
    return (
        <div className="fixed inset-0 z-50 overflow-y-auto bg-gray-900 bg-opacity-70 flex items-center justify-center backdrop-blur-sm p-4" onClick={onClose}>
            ...
            >
                <h2 className="text-2xl font-semibold mb-6 text-gray-900 dark:text-white border-b pb-2 border-gray-100 dark:border-gray-700">Edit Employee: {rowData.user || `ID: ${rowData.id}`}</h2>
                    ...
                    <div className={`grid ${gridCols} gap-4`}>
                        {dataKeys.map(key => (
                            <div key={key} className="col-span-1">
                                ...
                                >
                                    {formatLabel(key)} {key === 'id' && <span className="text-gray-500">(Read-only)</span>}
                                </label>
                                <input
                                    id={`edit-${key}`}
                                    ...
                                    className={`w-full p-2.5 border border-gray-300 rounded-lg dark:bg-gray-700 dark:border-gray-600 text-gray-800 dark:text-white focus:ring-blue-500 focus:border-blue-500 shadow-sm ${key === 'id' ? 'bg-gray-100 dark:bg-gray-700/50 cursor-not-allowed' : ''}`}
                                    required={key !== 'id'}
                                />
                            </div>
                        ))}
